...
from manyfold.model import r3, utils
...
def _make_renaming_matrices():
    ...
    all_correspondences = {res: np.arange(14, dtype=np.float32) for res in restype_3}
    for resname, swap in residue_constants.residue_atom_renaming_swaps.items():
        ...
        all_correspondences[resname] = correspondences.astype(np.float32)
    renaming_matrices = np.stack([all_matrices[restype] for restype in restype_3])
    renaming_correspondences = np.stack(
        [all_correspondences[restype] for restype in restype_3]
    )
    return renaming_matrices, renaming_correspondences.astype(np.int32)


RENAMING_MATRICES, RENAMING_CORRESPONDENCES = _make_renaming_matrices()
...
def get_alt_atom14(aatype, positions, mask):
    ...
    alternative_positions = jax.tree_map(
        lambda x: jnp.sum(x * renaming_transform, axis=1), positions
    )
    ...
    return alternative_positions, alternative_mask.astype(np.int32)


def get_alt_atom14_from_3d(aatype, positions, mask):
    """Get alternative atom14 positions.

    Constructs renamed atom positions for ambiguous residues.

    This is (hopefully!) the same as get_alt_atom14, except operating
    on 3D position representations, rather than r3.Vecs.

    Args:
      aatype: Amino acid at given position
      positions: Atom positions in atom14 representation, (N, 14, 3)
      mask: Atom masks in atom14 representation, (N, 14)
    Returns:
      renamed atom positions, renamed atom mask
    """
    # pick the transformation matrices for the given residue sequence
    # shape (num_res, 14, 14)
    renaming_corr = utils.batched_gather(
        jnp.asarray(RENAMING_CORRESPONDENCES), aatype
    ).astype(int)

    alternative_positions = positions[np.arange(len(positions))[:, None], renaming_corr]
    alternative_mask = mask[np.arange(len(positions))[:, None], renaming_corr]
