    ...
    def __call__(
        ...
        for head_name, head_config in sorted(self.config.heads.items()):
            ...
            head_factory = {
                ...
                "predicted_lddt": PredictedLDDTHead,  # SM 1.9.6
                "predicted_aligned_error": PredictedAlignedErrorHead,  # SM 1.9.7
...
class MSAColumnGlobalAttention(hk.Module):
    ...
    def __call__(self, msa_act, msa_mask, is_training=False):
        ...
        if self.config.subbatch_size_global:
            low_memory = True
            remat = is_training
        else:
            low_memory = not is_training
...
class PredictedAlignedErrorHead(hk.Module):
    ...
    def loss(self, value, batch):
        ...
        output = {"loss": loss, "aligned_error": error_dist2}
        return output
...
class DistogramHead(hk.Module):
    ...
    def __call__(self, representations, batch, is_training):
        ...
        return {"logits": logits, "bin_edges": breaks}
...
def _distogram_log_loss(logits, bin_edges, batch, num_bins):
    ...
    return {"loss": avg_error, "true_dist": jnp.sqrt(1e-6 + dist2)}
...
class OuterStackMean(hk.Module):
    """Computes mean outer stack and projection.

    Jumper et al. (2021) Suppl. Alg. 10 "OuterProductMean"
    """

    def __init__(
        self, config, global_config, num_output_channel, name="outer_stack_mean"
    ):
        super().__init__(name=name)
        self.global_config = global_config
        self.config = config
    ...
    def __call__(self, act, mask, is_training=True):
        """Builds OuterStack module.

        Arguments:
          act: Single representation, shape [1, N_res, c_m].
          mask: Single mask, shape [1, N_res].
          is_training: Whether the module is in training mode.
        ...
        # act --> [N_seq, N_res, N_res, 2*c_m]
        act = jax.vmap(
            jax.vmap(
                lambda a, b: jnp.concatenate([a, b], -1), in_axes=(1, None), out_axes=1
            ),
            in_axes=(None, 1),
            out_axes=1,
        )(left_act, right_act)

        # act --> [N_res, N_res, 2*c_m]
        act = act.mean(0)

        # act --> [N_res, N_res, c_z]
        # act = common_modules.Linear(
        #     self.num_output_channel, initializer="linear", name="output_projection"
        # )(act)
        act = hk.Sequential(
            [
                common_modules.Linear(
                    2 * self.num_output_channel,
                    initializer="relu",
                    name="output_projection_layer1",
                ),
                jax.nn.relu,
                common_modules.Linear(
                    self.num_output_channel, name="output_projection_layer2"
                ),
            ]
        )(act)
...
class EvoformerIteration(hk.Module):
    ...
    def __call__(self, activations, masks, is_training=True, safe_key=None):
        ...
        if not self.is_extra_msa:
            msa_row_atten_config = c.msa_row_attention_with_pair_bias
        else:
            msa_row_atten_config = c.msa_row_attention_with_pair_bias_extra_msa

        msa_act = dropout_wrapper_fn(
            MSARowAttentionWithPairBias(
                msa_row_atten_config,
                gc,
...
class EmbeddingsAndEvoformer(hk.Module):
    ...
    def __call__(self, batch, is_training, safe_key=None):
        ...
        if c.template.enabled:
            # Move template embedding into remat block
            def template_embedding(template_batch, pair_activations, mask_2d):
                ...
                return template_pair_representation

            if gc.use_remat:
                template_embedding = hk.remat(template_embedding)

            template_batch = {k: batch[k] for k in batch if k.startswith("template_")}
            template_pair_representation = template_embedding(
                template_batch, pair_activations, mask_2d
            )
