...
from manyfold.model.tf import shape_helpers, shape_placeholders, utils

# Pylint gets confused by the curry1 decorator because it changes the number
#   of arguments to the function.
# pylint:disable=no-value-for-parameter

NUM_RES_PLM = shape_placeholders.NUM_RES_PLM
...
def add_plm_features(protein):
    # Add aatype, residue index, seq mask, and crop seed for pLM processing.
    protein["aatype_plm"] = protein["aatype"]
    protein["residue_index_plm"] = protein["residue_index"]
    protein["seq_mask_plm"] = protein["seq_mask"]
    protein["random_crop_to_size_seed_plm"] = utils.make_random_seed()
    return protein
...
def make_fixed_size(
    ...
    num_res,
    num_templates=0,
    num_res_plm: Optional[int] = 0,  # plmfold
):
    """Guess at the MSA and sequence dimensions to make fixed size."""

    pad_size_map = {
        NUM_RES_PLM: num_res_plm,
        NUM_RES: num_res,
...
def random_crop_to_size(
    ...
    unclamped_fape_fraction: Optional[int] = None,
    crop_plm: Optional[bool] = False,
):
    """Crop randomly to `crop_size`, or keep as is if shorter than that.

    Note: unclamped_fape_fraction is added for training. It should be a fraction
    between 0 and 1 if specfied, and will result change between two methods of sampling
    the crop positions (see SM 1.2.8) as well as add the "use_clamped_fape" feature to
    the protein.  If unclamped_fape_fraction = None (default), then this transform will
    act as per the default behaviour in the released code.
    """
    ...
    if crop_plm:
        initial_seed = protein["random_crop_to_size_seed_plm"]
    else:
        initial_seed = protein["random_crop_to_size_seed"]
    seed_maker = utils.SeedMaker(initial_seed=initial_seed)
    ...
    if unclamped_fape_fraction:
        # Custom implementation, see SM 1.2.8 for details.
        use_clamped_fape = (
            ...
            > unclamped_fape_fraction
        )
        use_clamped_fape = tf.cast(use_clamped_fape, tf.int32)
        ...
        num_res_crop_start = tf.random.stateless_uniform(
            shape=(),
            minval=0,
            maxval=n - (1 - use_clamped_fape) * x + 1,
            dtype=tf.int32,
            seed=seed_maker(),
        )

    else:
        # This is the origional implementation as released, which we default to
        # if unclamped_fape_fraction is None.
        num_res_crop_start = tf.random.stateless_uniform(
            shape=(),
            minval=0,
    ...
    if unclamped_fape_fraction:
        protein["use_clamped_fape"] = use_clamped_fape
    return protein
